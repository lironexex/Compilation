...
CodeBuffer::CodeBuffer() : buffer(), globalDefs() {
    continue_list.push_back(NextList());
    break_list.push_back(NextList());
}
...
int CodeBuffer::emitCommand(const string &cmnd, const string &cmnt) {
	auto comment = "; " + cmnt;
	if (cmnt.empty()) {
		comment = "";
	}
	...
	stringstream stream;
	if (!cmnd.empty()) {
        stream << left << setw(80) << cmnd;
    } stream << comment;
	
    return emit(stream.str());
}
